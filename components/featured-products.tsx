...
import { RatingData } from "./product-list"
  ...
  useEffect(() => {
    const load = async () => {
      const entries = await Promise.all(
        products.map(async (p: any) => [p.id, await fetchRating(p.id)] as const
        ))
      ...
      >
        {products.map((product: any) => {
          ...
          return (
            ...
            >
              <Link href={`/san-pham/${product.category.name}/${product.id}`} className="block overflow-hidden">
              ...
              <div className="p-4">
                <div className="flex items-center mb-1">
                  <div className="flex">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <Star
                        key={star}
                        className={`h-3 w-3 ${star <= Math.floor(rating) ? "text-yellow-400 fill-yellow-400" : "text-gray-300"}`}
                      />
                    ))}
                  </div>
                  <span className="text-xs text-gray-500 ml-1"> {rating} ({count} đánh giá)</span>
                </div>
                ...
                <div className="mb-3">
                  ...
                  <Progress value={(product.sold / product.stock) * 100} className="h-1.5" />
                </div>
